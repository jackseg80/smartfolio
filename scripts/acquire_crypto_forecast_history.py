"""Acquire an isolated, provenance-rich daily history artifact."""
...
from services.forecasting.history_acquisition import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "config" / "crypto_forecast_history.json",
    )
    parser.add_argument(
        "--end-date",
        required=True,
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=PROJECT_ROOT / "outputs" / "crypto-forecast-history",
    )
...
async def run(args: argparse.Namespace) -> Path:
    ...
    acquired = []
    async with BinancePublicMarketClient(str(config["base_url"])) as client:
        for spec in specs:
    ...
    return write_acquisition_artifact(
        ...
        acquisition_code_sha256=file_sha256(
            PROJECT_ROOT / "services" / "forecasting" / "history_acquisition.py"
        ),
        end_date=args.end_date,
