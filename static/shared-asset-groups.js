...
function initializeTaxonomySync() {
  ...
  try {
    ...
    console.log('✅ Taxonomy data loaded synchronously on init:', Object.keys(KNOWN_ASSET_MAPPING).length, 'aliases,', GROUP_ORDER.length, 'groups');
  } catch (error) {
    console.warn('⚠️ Sync load on init failed, trying async...', error.message);

    // Fallback async si sync fail
    loadTaxonomyData()
      .then(data => {
        updateGlobalVariables(data);
        console.log('✅ Taxonomy data loaded asynchronously:', Object.keys(KNOWN_ASSET_MAPPING).length, 'aliases,', GROUP_ORDER.length, 'groups');
      })
      .catch(error => {
        console.warn('⚠️ Taxonomy async load failed, using fallback:', error.message);
...
async function loadTaxonomyData() {
  ...
  try {
    ...
    console.log('✅ Taxonomy data loaded from API:', Object.keys(taxonomyCache.aliases).length, 'aliases,', taxonomyCache.groups.length, 'groups');

    return taxonomyCache;
  } catch (error) {
    console.warn('⚠️ Taxonomy API unavailable, using fallback:', error.message);
...
function loadTaxonomyDataSync() {
  ...
  try {
    ...
    console.log('✅ Taxonomy data loaded sync from API:', Object.keys(taxonomyCache.aliases).length, 'aliases,', taxonomyCache.groups.length, 'groups');
  } catch (error) {
    console.warn('⚠️ Taxonomy sync API failed, using fallback:', error.message);
...
export function getAssetGroup(symbol) {
  ...
  if (Object.keys(KNOWN_ASSET_MAPPING).length === 0) {
    console.warn('⚠️ Taxonomy data not loaded yet, trying sync load...');
    try {
      loadTaxonomyDataSync();
    } catch (error) {
      console.warn('⚠️ Sync load failed, using fallback:', error.message);
      return autoClassifySymbolFallback(upperSymbol);
    }

    // Vérifier encore après le chargement sync
    if (Object.keys(KNOWN_ASSET_MAPPING).length === 0) {
      console.warn('⚠️ Sync load returned empty mapping, using fallback');
      return autoClassifySymbolFallback(upperSymbol);
    }
  }
...
export function forceReloadTaxonomy() {
  console.log('🔄 Forcing taxonomy reload...');
  taxonomyCache = null;
  cacheTimestamp = 0;
  ...
  console.log('✅ Taxonomy reload completed');
}
...
export async function debugClassification() {
  ...
  console.log('Total symbols:', Object.values(groups).flat().length);
  console.log('Aliases mapping:', aliases);
}
