...
async function computeMacroTargetsDynamic(ctx, rb, walletStats) {
  ...
  try {
    ...
    if (allocationResult && allocationResult.allocation) {
      ...
      (window.debugLogger?.debug || console.log)('🎯 V2 Dynamic targets computed (before Phase Engine):', targets);
      ...
      try {
        ...
        if (phaseEngineEnabled !== 'off') {
          ...
          if (phaseEngineEnabled === 'apply') {
            ...
            if (phaseResult && phaseResult.targets) {
              (window.debugLogger?.info || console.log)('✅ Phase Engine tilts applied:', {
        ...
        (window.debugLogger?.warn || console.warn)('⚠️ Phase Engine error:', error);
      }
