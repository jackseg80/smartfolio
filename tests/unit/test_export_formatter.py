...
class TestSaxoCsv:
    ...
    def test_csv_positions_header_row(self, saxo_data):
        ...
        formatter = ExportFormatter("saxo")
        output = formatter.to_csv(saxo_data)
        assert "Symbol,Instrument,Asset Class,Quantity,Market Value USD,Currency,Classification,Classification Basis,Entry Price" in output
    ...
    def test_csv_sectors_section(self, saxo_data):
        ...
        formatter = ExportFormatter("saxo")
        output = formatter.to_csv(saxo_data)
        assert "Classification,Value USD,Percentage,Asset Count" in output
...
class TestWealthCsv:
    ...
    def test_csv_header_comment(self, wealth_data):
        ...
        first_line = output.split("\n")[0]
        assert "Wealth Export" in first_line

    def test_csv_summary_section(self, wealth_data):
        """CSV contains summary with net worth, assets, liabilities."""
        formatter = ExportFormatter("wealth")
        output = formatter.to_csv(wealth_data)
        assert "Net Worth,650000.00" in output
        assert "Total Assets,750000.00" in output
        assert "Total Liabilities,100000.00" in output

    def test_csv_breakdown_section(self, wealth_data):
        """CSV contains breakdown by category with counts."""
        formatter = ExportFormatter("wealth")
        output = formatter.to_csv(wealth_data)
        assert "Category,ID,Name,Type,Value,Currency,Value USD,Acquisition Date,Notes" in output
        assert "Liquidity,liq-001,UBS Savings" in output
        assert "Tangible,tan-001,Apartment Zurich" in output
        assert "Insurance,ins-001,Life Insurance AXA" in output
        assert "Liability,lia-001,Mortgage" in output
    ...
    def test_csv_empty_categories(self):
        ...
        assert "Net Worth,0.00" in output
        assert "Category,ID,Name,Type,Value,Currency,Value USD,Acquisition Date,Notes" in output
...
class TestCryptoMarkdown:
    ...
    def test_md_footer(self, crypto_data):
        ...
        formatter = ExportFormatter("crypto")
        output = formatter.to_markdown(crypto_data)
        assert "*Generated by SmartFolio Export System*" in output
...
class TestSaxoMarkdown:
    ...
    def test_md_positions_table(self, saxo_data):
        ...
        formatter = ExportFormatter("saxo")
        output = formatter.to_markdown(saxo_data)
        assert "| Symbol | Instrument | Asset Class | Quantity | Market Value USD | Currency | Classification |" in output
    ...
    def test_md_sectors_table(self, saxo_data):
        ...
        formatter = ExportFormatter("saxo")
        output = formatter.to_markdown(saxo_data)
        assert "Classifications (GICS for equities; exposure for funds)" in output
...
class TestWealthMarkdown:
    ...
    def test_md_title(self, wealth_data):
        ...
        assert "Wealth Export" in output
    ...
    def test_md_items_by_category_tables(self, wealth_data):
        ...
        assert "Liquidities" in output
        assert "Tangible Assets" in output
...
class TestEdgeCases:
    ...
    def test_saxo_missing_fields_use_defaults(self):
        ...
        output = formatter.to_csv(data)
        # Default currency='USD', classification='Unclassified Equity'
        assert "USD" in output
        assert "Unclassified Equity" in output
    ...
    def test_wealth_csv_skips_empty_categories(self):
        ...
        output = formatter.to_csv(data)
        # The normalized table has the liquidity row and no empty tangible row.
        assert "Liquidity,x,Cash,cash" in output
        assert "Tangible," not in output
